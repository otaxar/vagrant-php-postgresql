...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.network "forwarded_port", guest: 80, host: 7105
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    mkdir postgres_tests
    cd postgres_tests
    mkdir queries

    cd /home/vagrant/dbsamples-0.1/world
    #sudo -u postgres createuser --createdb vagrant
    sudo -u postgres createuser -s $(whoami); createdb $(whoami)
    createdb DB7105
    psql DB7105 < world.sql

    cd /home/vagrant/postgres_tests/queries
    psql -d DB7105 -t -A -F"," -c "select lifeexpectancy from country where code='CHL'" > 2005_Chile_life.csv
 
    psql -d DB7105 -t -A -F"," -c "select language, percentage from countrylanguage where countrycode='CAN' LIMIT 5" > 2005_Canadian_lang.csv
    psql -d DB7105 -t -A -F"," -c "select language, percentage from countrylanguage where countrycode='CAN' AND language='Dutch'" >> 2005_Canadian_lang.csv
    psql -d DB7105 -t -A -F"," -c "select language, percentage from countrylanguage where countrycode='CAN' AND language='Spanish'" >> 2005_Canadian_lang.csv
